...
exports.login = async (req, res, next) => {
  ...
  //   }
    ...
    if (user && user.emailstatus == "not verified") {
      ...
      return res.status(200).json({ success: false, message: "You are not a verified user yet. Please verify your account." });
      ...
      await t.rollback(); // Rollback the transaction
      return res.status(202).json({ success: false, message: "this email is not registered with us. plz create an account" });
    }
  } catch (err) {
    await t.rollback(); // Rollback the transaction
    console.log(err);
